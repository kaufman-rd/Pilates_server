...
class PilatesController(acs.ACSController):
    ...
    def child_loop(self):
        self.position = sp.GetFPosition(self.handle, 0) * self.conversion_to_meters
        # self.velocity = sp.GetFVelocity(self.handle, 0)
        self.velocity = self.read_scalar("PREDICTION", 'float') * self.conversion_to_meters

        self.weight = self.read_scalar("WEIGHT", 'float')
        # self.total_weight = self.read_scalar("TOTAL_WEIGHT", 'float')
        self.total_weight = self.read_scalar("DCOM(0)", 'float') * -2
